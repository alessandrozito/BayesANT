...
BayesANT <- function(data,
  ...
  if(verbose == TRUE){cat("Estimating the Pitman-Yor parameters: Now. \n")}
  PYpars = suppressWarnings(fitPY_tree(data))
  if(verbose == TRUE){cat("Estimating the Pitman-Yor parameters: Done. \n")}

  ##### STEP 2 - extract the Prior probabilities given by the Pitman-Yor processes
  if(verbose == TRUE){cat("Computing prior probabilities: Now \n")}
  ...
  if(verbose == TRUE){cat("Computing prior probabilities: Done. \n")}
  ...
  if(typeseq == "aligned"){
    ...
    if(type_location == "single"){
      ...
      if(verbose == TRUE){cat("Extracting nucleotide location counts from sequences: Now. \n")}
      Nucl_counts = get_nucleotide_counts(nodes, DNAseq, nucl)
      if(verbose == TRUE){cat("Extracting nucleotide location counts from sequences: Done. \n")}
      ...
      if(verbose == TRUE){cat("Extracting 2mer location counts from sequences: Now. \n")}
      Nucl_counts = get_2mer_counts(nodes, DNAseq, nucl)
      if(verbose == TRUE){cat("Extracting 2mer location counts from sequences: Done. \n")}
    ...
    if(verbose == TRUE){cat("Extracting kmers from sequences: Now. \n")}
    Nucl_counts = get_Kmer_counts(k = kmers, nodes, DNAseq, nucl,
                                  adjust_Kmer_length = adjust_Kmer_length)
    if(verbose == TRUE){cat("Extracting kmers from sequences: Done. \n")}
  ...
  if(verbose == TRUE){cat("Computing hyperparameters: Now. \n")}

  if(typeseq == "aligned"){
    hyperparameters <- tune_hyperparameters(tree = Priorprobs[,1:lowest_level],
                                            Nucl_counts = Nucl_counts, verbose = verbose)
    if(verbose == TRUE){cat("Computing hyperparameters: Done. \n")}
  } else if (typeseq == "not aligned"){
    hyperparameters <- tune_hyperparameters_MultKmers(tree = Priorprobs[,1:lowest_level],
  ...
  if(verbose == TRUE){cat("Aggregating parameters: Now \n")}
  if(typeseq == "aligned"){
  ...
  if(verbose == TRUE){cat("Aggregating parameters: Done \n")}
